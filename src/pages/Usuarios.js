// v2.0 - muestra todos los usuarios correctamente
...
const ROL_CONFIG = {
  ...
  consulta: { label: 'Consulta', variant: 'success', desc: 'Solo lectura' },
};
...
export default function Usuarios({ data }) {
  ...
  const cargarUsuarios = async () => {
    setLoading(true);
    try {
      // Cargar perfiles con join a templos
      ...
      // Obtener emails desde auth.users via RPC o desde metadata
      // Como no tenemos acceso directo a auth.users desde el cliente,
      // usamos la sesión actual y guardamos el email en metadata
  ...
  const handleCrearUsuario = async () => {
    ...
    try {
      ...
      // Actualizar perfil con rol y templo correctos
      if (authData.user) {
            ...
            {loading ? (
              ...
              >
                ...
                <td style={{ padding: '13px 16px' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                    <div style={{
                      ...
                      background: u.rol === 'admin' ? 'rgba(28,43,75,0.12)' : u.rol === 'cobrador' ? 'var(--gold-pale)' : 'var(--success-bg)',
                ...
                <td style={{ padding: '13px 16px' }}>
                  <Button
                    size="sm"
                    variant={u.activo !== false ? 'ghost' : 'success'}
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title="Crear nuevo usuario">
        <div style={{ display: 'grid', gap: 16 }}>
          ...
          <div style={{
            ...
            <strong>Admin:</strong> gestión total.<br />
            <strong>Cobrador:</strong> registra cobranzas de su templo.<br />
            <strong>Consulta:</strong> solo lectura de todos los datos.
          </div>
