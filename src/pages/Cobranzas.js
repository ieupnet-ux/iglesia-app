// v4.0 - cobrador vinculado al usuario logueado
...
export default function Cobranzas({ data, registrarCobranza, eliminarCobranza, perfilActual }) {
  ...
  // Cobrador vinculado al usuario logueado
  const [miCobradorId, setMiCobradorId] = useState(null);
  const esAdmin = perfilActual?.rol === 'admin' || perfilActual?.rol === 'consulta';
  ...
  // ── Buscar cobrador vinculado al usuario logueado ─────────
  useEffect(() => {
    const buscarMiCobrador = async () => {
      if (!perfilActual?.id) return;

      // Buscar en cobradores por perfil_id
      ...
      if (cobVinculado?.length > 0) {
        ...
        // Si es cobrador pero no está vinculado, buscar por templo
    ...
    buscarMiCobrador();
  }, [perfilActual, cobradores, esAdmin]);
  ...
  // Deudas pendientes del miembro seleccionado
  ...
  const deudaSeleccionada = (id) => deudasAnuales.find(d => d.id === parseInt(id));
  ...
  const handleGuardar = async () => {
    ...
    try {
      ...
      setForm(f => ({
        ...f,
        miembro_id: '', aniosSeleccionados: '', numero_recibo: '', fecha: today,
        // mantener cobrador_id si es cobrador vinculado
  ...
  // Filtrar cobranzas — cobrador solo ve las suyas
  ...
  // Miembro del cobrador — si es cobrador, solo ve miembros de su templo
  const miembrosFiltrados = esAdmin
    ? miembros
    : miembros.filter(m => m.templo_id === perfilActual?.templo_id);

  // Cobrador actual vinculado
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', gap: 12, marginBottom: 20, flexWrap: 'wrap' }}>
        <input
          placeholder="Buscar por recibo, miembro…"
          value={busqueda}
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
            <thead>
              <tr style={{ background: 'var(--gray-50)', borderBottom: '1px solid var(--gray-200)' }}>
                {['Fecha','N° Recibo','Año','Cobrador','Templo','Miembro','Categoría','Monto',''].map(h => (
                  <th key={h} style={{
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title="Registrar cobranza" width={540}>
        <div style={{ display: 'grid', gap: 16 }}>
          ...
          {/* Cobrador — fijo si es cobrador vinculado, editable si es admin */}
          <FormField label="Cobrador">
            {miCobradorId && !esAdmin ? (
              // Cobrador vinculado — no puede cambiarse
              <div style={{
              ...
              // Admin puede elegir cualquier cobrador
                ...
                <div style={{ border: '1.5px solid var(--gray-200)', borderRadius: 8, overflow: 'hidden' }}>
                  <div style={{
                    ...
                    <button onClick={seleccionarTodos} style={{
                      ...
                      fontSize: 12, fontWeight: 600, color: 'var(--navy)', textDecoration: 'underline', padding: 0,
                  ...
                  {deudasDelMiembro.map(d => {
                    ...
                    return (
                      <div key={d.id} style={{
                        display: 'grid', gridTemplateColumns: '32px 60px 1fr 120px',
                        alignItems: 'center', gap: 10,
                        padding: '10px 14px', borderBottom: '1px solid var(--gray-100)',
                        background: sel ? 'rgba(28,43,75,0.03)' : 'transparent',
                      }}>
                        ...
                        {sel ? (
                          <input type="number" min="1" max={d.saldo}
